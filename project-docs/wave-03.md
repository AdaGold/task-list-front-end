...
- In the `App`, use the `useEffect` hook to make an API call to get the list of tasks in the database when the React app is loaded.
- Update the `updateTask` callback function (the function that toggles the state of completion) in the `App` such that updating a task in the front end makes a change to the state of the task in the database. 
- Update the `deleteTask` callback function (the function that deletes a task) in the `App` such that deleting a task in the front end, deletes the task from the database. 
